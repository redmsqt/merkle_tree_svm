...
impl MerkleTree {
    ...
    pub fn insert<T: AsRef<[u8]>>(&mut self, item: T) {
        ...
        // Rebalance the tree
        let mut level_len = self.leaf_count;
        let mut level_start = if self.nodes.len() >= level_len {
            self.nodes.len() - level_len
        } else {
            0
        };
        ...
        while level_len > 0 {
            for i in 0..level_len / 2 {
                let lsib = &self.nodes[level_start + 2 * i];
    
                let rsib_index = level_start + 2 * i + 1;
                let rsib = if rsib_index < self.nodes.len() {
                    &self.nodes[rsib_index]
                } else {
                    lsib // Duplicate the last element if odd
                };
...
mod tests {
    ...
    #[test]
    fn test_insert_leaf() {
        let mut mt = MerkleTree::new(TEST);
        let new_leaf = b"new_leaf";

        // Retrieve the root before insertion
        let initial_root = mt.get_root().unwrap().to_owned();

        // Insert a new leaf
        mt.insert(new_leaf); // Emprunt mutable ici

        // Check that the number of leaves has increased
        assert_eq!(mt.leaf_count, TEST.len() + 1);

        // Check that the root has changed after insertion
        let new_root = mt.get_root().unwrap(); 
    ...
    fn test_insert_multiple_leaves() {
        let mut mt = MerkleTree::new(TEST);
        let new_leaves = vec![b"leaf1", b"leaf2", b"leaf3", b"leaf4"];
        ...
        // Check that the number of leaves has increased
        assert_eq!(mt.leaf_count, TEST.len() + new_leaves.len());
